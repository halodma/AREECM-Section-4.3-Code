...
print()
element = input("Enter your element here (e.g., H, He, Li): ").strip()

if element in areecm_digital_dictionary:

    total_electrons, orbitals = areecm_digital_dictionary[element]

    print(f"\nElement: {element}")
    print(f"Total electrons: {total_electrons}")
    print("Electrons in each orbital (minimum possible, max possible):")
    ...
    for orbital, values in orbitals.items():
        pmin, pmax = values
        n_min = total_electrons * pmin
        n_max = total_electrons * pmax

        orbital_results[orbital] = {"pmin": pmin, "pmax": pmax, "n_min": n_min, "n_max": n_max}

        total_max_sum += n_max  # for orbital dominance weight

        # extract principal quantum number
        # example: "4f" → n = 4
        shell_n = int(orbital[0])
        if shell_n > max_n_shell:
            max_n_shell = shell_n

        print(f"  {orbital}: {n_min:.4f} / {n_max:.4f}")

    print("\nOrbital Dominance Weight (fraction of total max distribution):")
    for orbital, data in orbital_results.items():
        w = data["n_max"] / total_max_sum if total_max_sum != 0 else 0
        print(f"  {orbital}: W = {w:.4f}")
